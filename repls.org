...
** nREPL
   ...
   [[https://nrepl.org/][nREPL]] stands for "network REPL", and while this may sound pretty similar to a
...
*** Custom Middleware
   ...
   asynchronous operation. A handler can send several messages back to the client based on a
   ...
   To see this middleware in action, make sure you have ~nrepl~ in your
   ...
   #+BEGIN_SRC clojure
     ...
     (nrepl/start-server :handler (nrepl/default-handler #'wrap-classpath))
   #+END_SRC
...
**** Notable middleware
     ...
     Many more operations are added by [[https://github.com/clojure-emacs/refactor-nrepl][refactor-nrepl]].

     [[https://github.com/nrepl/piggieback][Piggieback]] allows using nREPL with ClojureScript, see the Piggieback section
...
*** Clients
    ...
    a dedicated client library. The [[https://github.com/nrepl/nrepl][nREPL]] package contains both the nREPL
    server and a client implementation that you can use from Clojure.

    - [[https://github.com/clojure-emacs/cider-nrepl][CIDER-nREPL]] contains an nREPL client implemented in Emacs Lisp. If you want to
    ...
    The nREPL manual lists [[https://nrepl.org/nrepl/usage/clients.html][editors and tools that support nREPL]].
...
*** ClojureScript clients for nREPL
    ...
    would be to use HTTP over JSON, which is what [[https://github.com/nrepl/drawbridge][Drawbridge]] provides. It is
...
** Piggieback
   ...
   Calling ~(cider.piggieback/cljs-repl (repl-env))~ from the REPL spurs
   ...
   ~(cider.piggieback/cljs-repl)~, passing in the ClojureScript repl-env you
